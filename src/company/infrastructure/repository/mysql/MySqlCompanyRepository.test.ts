...
describe('adding company', () => {
  ...
  it('should add company to database when not already exist', async () => {
    ...
    await CompanyRepositoryVerification
      ...
      .shouldContainCompanyWithName(AN_ID, A_NAME);
  });
});
...
describe('find required company', () => {
  it('should throw an error when company does not exist in database', async () => {
    ...
    expect(repository.findRequired(A_COMPANY.getID())).rejects.toThrow(`Unable to find company with id ${AN_ID}`);
  });
...
describe('delete company', () => {
  it('should throw an error when company does not exist in database', async () => {
    ...
    expect(repository.delete(A_COMPANY.getID())).rejects.toThrow(`Unable to delete company with id ${AN_ID}`);
  });
